<?php
// ARQUIVO: estoque_aneis.php
// Gerenciamento de estoque para a categoria Anéis, com lógica de inserção, exibição, exclusão e edição.
...
if (isset($_GET['acao']) && $_GET['acao'] == 'excluir' && isset($_GET['id'])) {
    ...
    if ($id_para_excluir) {
        try {
            // -- ATENÇÃO: Tabela 'aneis' --
            $sql_excluir = "DELETE FROM aneis WHERE id = :id";
            $stmt_excluir = $pdo->prepare($sql_excluir);
            $stmt_excluir->bindParam(':id', $id_para_excluir, PDO::PARAM_INT);
            $stmt_excluir->execute();
            
            if ($stmt_excluir->rowCount() > 0) {
                $mensagem_status = "Sucesso! Anel (ID: {$id_para_excluir}) excluído do estoque.";
                $classe_status = "success";
            } else {
                $mensagem_status = "Erro: Nenhum anel encontrado com o ID {$id_para_excluir} para exclusão.";
                $classe_status = "error";
            }
        } catch (\PDOException $e) {
            $mensagem_status = "Erro no banco de dados ao excluir: Tabela 'aneis' não existe. " . $e->getMessage();
            $classe_status = "error";
        }
        // Redireciona para limpar o parâmetro GET e evitar re-exclusão ao atualizar a página
        header("Location: estoque_aneis.php?status={$classe_status}&msg=" . urlencode($mensagem_status));
        exit();
    }
}
    ...
    if (!$nome || !$descricao || $preco === false || $quantidade_estoque === false) {
        ...
        try {
            ...
            // --- AÇÃO DE INSERÇÃO (via POST) ---
                ...
                // -- ATENÇÃO: Tabela 'aneis' --
                $sql = "INSERT INTO aneis (nome, descricao, preco, caminho_imagem, quantidade_estoque) 
            ...
            $mensagem_status = "Sucesso! Anel '{$nome_produto_op}' {$operacao} ao estoque.";
            $classe_status = "success";

            // Redireciona para recarregar a lista com a nova informação
            header("Location: estoque_aneis.php?status={$classe_status}&msg=" . urlencode($mensagem_status));
            exit();

        } catch (\PDOException $e) {
            $mensagem_status = "Erro no banco de dados: Tabela 'aneis' não existe ou erro SQL: " . $e->getMessage();
            $classe_status = "error";
        } catch (\Exception $e) {
...
try {
    ...
    // -- ATENÇÃO: Tabela 'aneis' --
    $sql_select = "SELECT id, nome, descricao, preco, caminho_imagem, quantidade_estoque FROM aneis ORDER BY nome ASC";
    $stmt_select = $pdo->query($sql_select);
    $aneis = $stmt_select->fetchAll(PDO::FETCH_ASSOC);

    // Converte o array de aneis para JSON para uso no JavaScript
    $aneis_json = json_encode($aneis);
    
} catch (\PDOException $e) {
    // Caso a tabela não exista, inicializa o array como vazio para evitar o erro fatal
    $aneis = []; 
    $aneis_json = "[]";
    // E define uma mensagem de erro se ainda não houver uma
    if (empty($mensagem_status)) {
        $mensagem_status = "ERRO FATAL: Tabela 'aneis' não foi encontrada. Por favor, crie a tabela no seu banco de dados.";
        $classe_status = "error";
    }
}
...
<head>
    ...
    <title>Estoque - Anéis | <?= htmlspecialchars($usuario_nome) ?></title>
...
/* ------------------------------------- */
/* Corrigido para botões com ícones */
.btn-editar, .btn-excluir-tabela {
    padding: 8px 12px;
    border: none;
    border-radius: 4px;
    cursor: pointer;
    font-size: 1em; /* Para o ícone */
    margin: 0 3px;
...
.btn-cancelar,
.btn-adicionar,
.btn-salvar-edicao { /* Adicionado botão salvar edição */
    padding: 10px 18px;
    border: none;
    border-radius: 6px;
    cursor: pointer;
...
.btn-adicionar:hover {
    background-color: #3f281dff;
}

.btn-salvar-edicao { /* Novo estilo para salvar edição */
    background-color: #3a1e10; /* Azul */
    color: white;
    font-size: 12px;
}
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            <div class="estoque-header">
                <h1>Estoque de Anéis</h1>
                <p style="color: #cfa266;">Veja o estoque atual e faça ajustes. Lembre-se de criar a tabela 'aneis' no banco.</p>
            </div>
            ...
            <div class="estoque-tabela-container">
                <table class="tabela-estoque">
                    ...
                    <tbody>
                        <?php if (!empty($aneis)): ?>
                            <?php foreach ($aneis as $anel): ?>
                                <tr>
                                    <td><?= htmlspecialchars($anel['id']) ?></td>
                                    <td><?= htmlspecialchars($anel['nome']) ?></td>
                                    <td><?= htmlspecialchars($anel['descricao']) ?></td>
                                    <td>R$ <?= number_format($anel['preco'], 2, ',', '.') ?></td>
                                    <td><?= htmlspecialchars($anel['quantidade_estoque']) ?></td>
                                    <td class="col-acoes">
                                        <button class="btn-editar" data-id="<?= $anel['id'] ?>"><i class="fas fa-edit"></i></button>
                                        <button class="btn-excluir-tabela" data-id="<?= $anel['id'] ?>"><i class="fas fa-trash"></i></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" style="text-align: center;">Nenhum anel encontrado no estoque.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
        ...
        <div class="modal-overlay" id="modal-novo-produto">
            <div class="modal-content">
                <h2>Adicionar Novo Anel</h2>
                <form id="form-novo-produto" action="estoque_aneis.php" method="POST">
                    
                    <div class="form-group">
                        <label for="produto-nome">Nome do Produto:</label>
                        <input type="text" id="produto-nome" name="nome" required placeholder="ex: Anel Solitário Ouro">
                    </div>
                    <div class="form-group">
                        <label for="produto-descricao">Descrição:</label>
                        <textarea id="produto-descricao" name="descricao" required placeholder="ex: Ouro 18k com diamante central de 0.5ct"></textarea>
                    </div>
                    <div class="form-group">
                        <label for="produto-preco">Preço (Ex: 1599.99):</label>
                        <input type="number" id="produto-preco" name="preco" step="0.01" required placeholder="ex: 1899.90">
                    </div>
                    <div class="form-group">
                        <label for="produto-img">Caminho da Imagem:</label>
                        <input type="text" id="produto-img" name="caminho_imagem" required placeholder="ex: ../imagens/Aneis/SolitarioOuro.jpg">
                    </div>
                    <div class="form-group">
                    ...
                    <input type="hidden" name="acao" value="adicionar"> <div class="modal-actions">
        ...
        <div class="modal-overlay" id="modal-editar-produto">
            <div class="modal-content">
                <h2>Editar Anel</h2>
                <form id="form-editar-produto" action="estoque_aneis.php" method="POST">
                    ...
                    <div class="form-group">
                        <label for="editar-produto-preco">Preço (Ex: 1599.99):</label>
    ...
    <script>
        ...
        // Isso permite buscar os detalhes de um produto sem uma nova requisição ao servidor
        const aneisData = <?= $aneis_json ?>;

        // 2. Lógica para Modais (Adicionar e Editar)
        ...
        // Modal Editar - Fechar clicando fora
        ...
        document.querySelectorAll('.btn-editar').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const produto = aneisData.find(a => a.id == id); // Encontra o produto no array JS
        ...
        document.querySelectorAll('.btn-excluir-tabela').forEach(button => {
            button.addEventListener('click', function() {
                const id = this.getAttribute('data-id');
                const produto = aneisData.find(a => a.id == id);
                const nomeProduto = produto ? produto.nome : `(ID: ${id})`;

                // Confirmação via JavaScript antes de enviar a requisição
                if (confirm(`Tem certeza que deseja EXCLUIR o anel "${nomeProduto}"? Esta ação é irreversível!`)) {
                    // Redireciona para o script PHP com os parâmetros de exclusão
                    window.location.href = `estoque_aneis.php?acao=excluir&id=${id}`;
                }
            });
        });
        
        // Nota: O código JavaScript original para abrir/fechar o modal de adição foi atualizado para usar os novos IDs 
        // ('btn-abrir-modal-adicionar', 'btn-fechar-modal-adicionar')
    </script>
</body>
</html>
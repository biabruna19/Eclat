<?php
// ARQUIVO: estoque_relogios.php
// Gerenciamento de estoque para a categoria Relógios, com lógica de inserção, exibição, exclusão e edição.
...
if (isset($_GET['acao']) && $_GET['acao'] == 'excluir' && isset($_GET['id'])) {
    ...
    if ($id_para_excluir) {
        try {
            $sql_excluir = "DELETE FROM relogios WHERE id = :id";
            $stmt_excluir = $pdo->prepare($sql_excluir);
            $stmt_excluir->bindParam(':id', $id_para_excluir, PDO::PARAM_INT);
            $stmt_excluir->execute();
            
            if ($stmt_excluir->rowCount() > 0) {
                $mensagem_status = "Sucesso! Relógio (ID: {$id_para_excluir}) excluído do estoque.";
                $classe_status = "success";
            } else {
                $mensagem_status = "Erro: Nenhum relógio encontrado com o ID {$id_para_excluir} para exclusão.";
                $classe_status = "error";
            }
        } catch (\PDOException $e) {
            $mensagem_status = "Erro no banco de dados ao excluir: " . $e->getMessage();
            $classe_status = "error";
        }
        // Redireciona para limpar o parâmetro GET e evitar re-exclusão ao atualizar a página
        header("Location: estoque_relogios.php?status={$classe_status}&msg=" . urlencode($mensagem_status));
        exit();
    }
}
    ...
    if (!$nome || !$descricao || $preco === false || $quantidade_estoque === false) {
        ...
        try {
            ...
            // --- AÇÃO DE INSERÇÃO (via POST) ---
                ...
                $sql = "INSERT INTO relogios (nome, descricao, preco, caminho_imagem, quantidade_estoque) 
            ...
            $mensagem_status = "Sucesso! Relógio '{$nome_produto_op}' {$operacao} ao estoque.";
            $classe_status = "success";
            ...
            $mensagem_status = "Erro no banco de dados: Tabela 'relogios' não existe ou erro SQL: " . $e->getMessage();
            $classe_status = "error";
        } catch (\Exception $e) {
...
try {
    ...
    $sql_select = "SELECT id, nome, descricao, preco, caminho_imagem, quantidade_estoque FROM relogios ORDER BY nome ASC";
    $stmt_select = $pdo->query($sql_select);
    $relogios = $stmt_select->fetchAll(PDO::FETCH_ASSOC);

    // Converte o array de relógios para JSON para uso no JavaScript
    $relogios_json = json_encode($relogios);
    ...
    if (empty($mensagem_status)) {
        $mensagem_status = "ERRO FATAL: Tabela 'relogios' não foi encontrada. Por favor, crie a tabela no seu banco de dados.";
        $classe_status = "error";
    }
}


// 4. CONFIGURAÇÃO DO DASHBOARD E MENU (Sem alterações)
$usuario_nome = "Luiza"; 
$pagina_atual = "Relógios"; 
...
<head>
    ...
    <title>Estoque - Relógios | <?= htmlspecialchars($usuario_nome) ?></title>
...
.btn-editar, .btn-excluir-tabela {
    padding: 8px 12px; /* Ajustado para ícone */
    border: none;
    border-radius: 4px;
    cursor: pointer;
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            <div class="estoque-header">
                <h1>Estoque de Relógios</h1>
                <p style="color: #cfa266;">Veja o estoque atual e faça ajustes. Lembre-se de criar a tabela 'relogios' no banco.</p>
            </div>
                    ...
                    <tbody>
                        <?php if (!empty($relogios)): ?>
                            <?php foreach ($relogios as $relogio): ?>
                                <tr>
                                    <td><?= htmlspecialchars($relogio['id']) ?></td>
                                    <td><?= htmlspecialchars($relogio['nome']) ?></td>
                                    <td><?= htmlspecialchars($relogio['descricao']) ?></td>
                                    <td>R$ <?= number_format($relogio['preco'], 2, ',', '.') ?></td>
                                    <td><?= htmlspecialchars($relogio['quantidade_estoque']) ?></td>
                                    <td class="col-acoes">
                                        <button class="btn-editar" data-id="<?= $relogio['id'] ?>"><i class="fas fa-edit"></i></button>
                                        <button class="btn-excluir-tabela" data-id="<?= $relogio['id'] ?>"><i class="fas fa-trash"></i></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" style="text-align: center;">Nenhum relógio encontrado no estoque.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
        ...
        <div class="modal-overlay" id="modal-novo-produto">
            <div class="modal-content">
                ...
                <form id="form-novo-produto" action="estoque_relogios.php" method="POST">
                    
                    <div class="form-group">
                        <label for="produto-nome">Nome do Produto:</label>
                        <input type="text" id="produto-nome" name="nome" required placeholder="ex: Relógio Angers">
                    </div>
                    <div class="form-group">
                        <label for="produto-descricao">Descrição:</label>
                        <textarea id="produto-descricao" name="descricao" required placeholder="ex: Masculino em Prata 925, pulseira em couro legítimo preto."></textarea>
                    </div>
                    <div class="form-group">
                        <label for="produto-preco">Preço (Ex: 1599.99):</label>
                        <input type="number" id="produto-preco" name="preco" step="0.01" required placeholder="ex: 2499.00">
                    </div>
                    <div class="form-group">
                        <label for="produto-img">Caminho da Imagem:</label>
                        <input type="text" id="produto-img" name="caminho_imagem" required placeholder="ex: ../imagens/Relogios/RelogioAngers.jpg">
                    </div>
                    <div class="form-group">
        ...
        <div class="modal-overlay" id="modal-editar-produto">
            <div class="modal-content">
                ...
                <form id="form-editar-produto" action="estoque_relogios.php" method="POST">
    ...
    <script>
        ...
        const relogiosData = <?= $relogios_json ?>;
        ...
        document.querySelectorAll('.btn-editar').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const produto = relogiosData.find(r => r.id == id); // Encontra o produto no array JS
        ...
        document.querySelectorAll('.btn-excluir-tabela').forEach(button => {
            button.addEventListener('click', function() {
                const id = this.getAttribute('data-id');
                const produto = relogiosData.find(r => r.id == id);
                const nomeProduto = produto ? produto.nome : `(ID: ${id})`;

                // Confirmação via JavaScript antes de enviar a requisição
                if (confirm(`Tem certeza que deseja EXCLUIR o relógio "${nomeProduto}"? Esta ação é irreversível!`)) {
                    // Redireciona para o script PHP com os parâmetros de exclusão
                    window.location.href = `estoque_relogios.php?acao=excluir&id=${id}`;
                }
            });
        });

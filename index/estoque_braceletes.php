<?php
// ARQUIVO: estoque_braceletes.php
// Gerenciamento de estoque para a categoria Braceletes, com lógica de inserção, exibição, exclusão e edição.
...
if (isset($_GET['acao']) && $_GET['acao'] == 'excluir' && isset($_GET['id'])) {
    ...
    if ($id_para_excluir) {
        try {
            // -- ATENÇÃO: Tabela 'braceletes' --
            $sql_excluir = "DELETE FROM braceletes WHERE id = :id";
            $stmt_excluir = $pdo->prepare($sql_excluir);
            $stmt_excluir->bindParam(':id', $id_para_excluir, PDO::PARAM_INT);
            $stmt_excluir->execute();
            
            if ($stmt_excluir->rowCount() > 0) {
                $mensagem_status = "Sucesso! Bracelete (ID: {$id_para_excluir}) excluído do estoque.";
                $classe_status = "success";
            } else {
                $mensagem_status = "Erro: Nenhum bracelete encontrado com o ID {$id_para_excluir} para exclusão.";
                $classe_status = "error";
            }
        } catch (\PDOException $e) {
            $mensagem_status = "Erro no banco de dados ao excluir: Tabela 'braceletes' não existe. ";
            $classe_status = "error";
        }
        // Redireciona para limpar o parâmetro GET e evitar re-exclusão
        header("Location: estoque_braceletes.php?status={$classe_status}&msg=" . urlencode($mensagem_status));
        exit();
    }
}
    ...
    if (!$nome || !$descricao || $preco === false || $quantidade_estoque === false) {
        ...
        try {
            ...
            // --- AÇÃO DE INSERÇÃO (via POST) ---
                ...
                // -- ATENÇÃO: Tabela 'braceletes' --
                $sql = "INSERT INTO braceletes (nome, descricao, preco, caminho_imagem, quantidade_estoque) 
            ...
            $mensagem_status = "Sucesso! Bracelete '{$nome_produto_op}' {$operacao} ao estoque.";
            $classe_status = "success";
            ...
            $mensagem_status = "Erro no banco de dados: Tabela 'braceletes' não existe ou erro SQL: " . $e->getMessage();
            $classe_status = "error";
        } catch (\Exception $e) {
...
try {
    // -- ATENÇÃO: Tabela 'braceletes' --
    $sql_select = "SELECT id, nome, descricao, preco, caminho_imagem, quantidade_estoque FROM braceletes ORDER BY nome ASC";
    $stmt_select = $pdo->query($sql_select);
    $braceletes = $stmt_select->fetchAll(PDO::FETCH_ASSOC);

    // Converte o array de braceletes para JSON para uso no JavaScript
    $braceletes_json = json_encode($braceletes);
    ...
    if (empty($mensagem_status)) {
        $mensagem_status = "ERRO FATAL: Tabela 'braceletes' não foi encontrada. Por favor, crie a tabela no seu banco de dados.";
        $classe_status = "error";
    }
}
...
$usuario_nome = "Luiza"; 
// -- ATENÇÃO: Página atual 'Braceletes' --
$pagina_atual = "Braceletes"; 
...
$categorias_menu = [
    ...
    ["nome" => "Braceletes", "link" => "estoque_braceletes.php"], // Link correto
];
?>
...
<head>
    ...
    <title>Estoque - Braceletes | <?= htmlspecialchars($usuario_nome) ?></title>
...
.btn-editar:hover { background-color: #ceb89bff; }
.btn-excluir-tabela { background-color: #3a1e10; color: white; }
.btn-excluir-tabela:hover { background-color: #3f2b21ff; }
...
.btn-novo-produto:hover { background-color: #d1b999ff; }
.btn-cancelar { background-color: #3a1e10; color: white; }
.btn-cancelar:hover { background-color: #442c1fff; }
.btn-adicionar { background-color: #3a1e10; color: white; font-size: 12px; }
.btn-adicionar:hover { background-color: #422b1fff; }
.btn-salvar-edicao { background-color: #3a1e10; color: white; font-size: 12px; }
.btn-salvar-edicao:hover { background-color: 422b1fff; }
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            <div class="estoque-header">
                <h1>Estoque de Braceletes</h1>
                <p style="color: #cfa266;">Veja o estoque atual e faça ajustes. Lembre-se de criar a tabela 'braceletes' no banco.</p>
            </div>
            ...
            <div class="estoque-tabela-container">
                <table class="tabela-estoque">
                    ...
                    <tbody>
                        <?php if (!empty($braceletes)): ?>
                            <?php foreach ($braceletes as $bracelete): ?>
                                <tr>
                                    <td><?= htmlspecialchars($bracelete['id']) ?></td>
                                    <td><?= htmlspecialchars($bracelete['nome']) ?></td>
                                    <td><?= htmlspecialchars($bracelete['descricao']) ?></td>
                                    <td>R$ <?= number_format($bracelete['preco'], 2, ',', '.') ?></td>
                                    <td><?= htmlspecialchars($bracelete['quantidade_estoque']) ?></td>
                                    <td class="col-acoes">
                                        <button class="btn-editar" data-id="<?= $bracelete['id'] ?>"><i class="fas fa-edit"></i></button>
                                        <button class="btn-excluir-tabela" data-id="<?= $bracelete['id'] ?>"><i class="fas fa-trash"></i></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" style="text-align: center;">Nenhum bracelete encontrado no estoque.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
        ...
        <div class="modal-overlay" id="modal-novo-produto">
            <div class="modal-content">
                <h2>Adicionar Novo Bracelete</h2>
                <form id="form-novo-produto" action="estoque_braceletes.php" method="POST">
                    
                    <div class="form-group">
                        <label for="produto-nome">Nome do Produto:</label>
                        <input type="text" id="produto-nome" name="nome" required placeholder="ex: Bracelete de Prata Bali">
                    </div>
                    <div class="form-group">
                        <label for="produto-descricao">Descrição:</label>
                        <textarea id="produto-descricao" name="descricao" required placeholder="ex: Prata 925 com detalhes em baixo relevo"></textarea>
                    </div>
                    <div class="form-group">
                        <label for="produto-preco">Preço (Ex: 599.99):</label>
                        <input type="number" id="produto-preco" name="preco" step="0.01" required placeholder="ex: 599.90">
                    </div>
                    <div class="form-group">
                        <label for="produto-img">Caminho da Imagem:</label>
                        <input type="text" id="produto-img" name="caminho_imagem" required placeholder="ex: ../imagens/Braceletes/PrataBali.jpg">
                    </div>
                    <div class="form-group">
        ...
        <div class="modal-overlay" id="modal-editar-produto">
            <div class="modal-content">
                ...
                <form id="form-editar-produto" action="estoque_braceletes.php" method="POST">
                    ...
                    <div class="form-group">
                        <label for="editar-produto-preco">Preço (Ex: 599.99):</label>
    ...
    <script>
        ...
        // ATENÇÃO: VARIÁVEL AGORA USA OS DADOS DE BRACELETES
        const braceletesData = <?= $braceletes_json ?>;
        ...
        document.querySelectorAll('.btn-editar').forEach(button => {
            button.addEventListener('click', function() {
                ...
                // ATENÇÃO: Busca no array correto (braceletesData)
                const produto = braceletesData.find(b => b.id == id); 
        ...
        document.querySelectorAll('.btn-excluir-tabela').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (confirm(`Tem certeza que deseja EXCLUIR o bracelete "${nomeProduto}"? Esta ação é irreversível!`)) {
                    // Redireciona para o script PHP com os parâmetros de exclusão
                    window.location.href = `estoque_braceletes.php?acao=excluir&id=${id}`;
                }
            });
        });

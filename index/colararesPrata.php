<?php
// ARQUIVO: index/colaresPrata.php
...
function getFavoritesFromCookie() {
    if (isset($_COOKIE[FAVORITES_KEY])) {
        ...
        // array_values garante que a lista esteja reindexada, útil se o cookie foi alterado
...
// ----------------------------------------------------------------------------------
// 3. Catálogo de Produtos (Mock Data - IDs ÚNICOS são CRUCIAIS)
// ----------------------------------------------------------------------------------
$PRODUTOS_PRATA = [
    [
...
<head>
    ...
    <title>Éclat Joias - Colares Prata</title>
  ...
  <header class="top-nav">
   ...
   <a href="../index/carrinho.php"> 
    ...
    <section class="catalogo">
        ...
        <div id="versailles" class="collection-section section-dourado">
            <h2>Coleção Versailles</h2>
            <p>Prata em sua forma mais pura. Colares que unem sofisticação e simplicidade em perfeita harmonia.</p>

            <div class="cards-grid" role="list" aria-label="Colares da Coleção Versailles">
                
                <?php foreach ($PRODUTOS_PRATA as $produto): 
                    // 4. Lógica para definir o estado inicial do coração (preenchido ou vazio)
                    $is_favorito = in_array($produto['id'], $favoritos_ids);

                    // CLASSE CRÍTICA: 'js-toggle-favorite' NO BOTÃO/LINK!
                    // A classe 'favorited' será usada para estilização no CSS/JS
                    $fav_btn_class = $is_favorito ? 'favorited' : ''; 
                    
                    // Ícone Inicial: 'fas' (preenchido) ou 'far' (vazio)
